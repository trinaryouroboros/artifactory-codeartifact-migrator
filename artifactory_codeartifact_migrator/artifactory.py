...
def artifactory_package_search(args, package, repository):
    ...
    package_search = artifactory_http_call(
        args, "/api/storage/" + repository + "/" + package
    )
    success = False
...
def artifactory_package_binary_search(args, package_dict):
    ...
    if package_dict["type"] == "pypi":
        ...
        binary_search = artifactory_http_call(
            args,
            f"/api/storage/{package_dict['repository']}/{package_dict['package'].split('/')[-1]}?list&deep=1",
        )
        if binary_search.get("files"):
            ...
            logger.info(
                f"No files found in Artifactory for {package_dict['repository']} {package_dict['package']}"
            )
    else:
        binary_search = artifactory_http_call(
            args,
            "/api/search/artifact?name="
            + package_dict["package"].split("/")[-1]
            + "&repos="
            ...
            if "/" + package_dict.get("package") + "/" in i["uri"]:
                if package_dict.get("version"):
                    if package_dict["type"] == "npm":
                        ...
                        logger.critical(
                            f"ERROR: Package type {package_dict['type']} not supported: {package_dict}"
                        )
                        sys.exit(1)
                else:
                    binaries.append(i["uri"])
...
def artifactory_npm_metadata_fetch(args, package_dict):
    ...
    """
    response = artifactory_http_call(
        args,
        "/"
        + package_dict["repository"]
        + "/.npm/"
