...
def dynamodb_create_packages_table(db_file):
    dynamodb.create_table(
        ...
        KeySchema=[{"AttributeName": "packagekey", "KeyType": "HASH"}],
        AttributeDefinitions=[
            {"AttributeName": "packagekey", "AttributeType": "S"},
            {"AttributeName": "package", "AttributeType": "S"},
            {"AttributeName": "repository", "AttributeType": "S"},
            {"AttributeName": "repositorypackage", "AttributeType": "S"},
        ...
        GlobalSecondaryIndexes=[
            {
                ...
                "KeySchema": [
                    ...
                    {"AttributeName": "package", "KeyType": "RANGE"},
                ],
            ...
            },
            {
                ...
                "KeySchema": [
                    {"AttributeName": "repositorypackage", "KeyType": "HASH"},
...
def dynamodb_create_repositories_table(db_file):
    dynamodb.create_table(
        ...
        KeySchema=[{"AttributeName": "repositorykey", "KeyType": "HASH"}],
        AttributeDefinitions=[{"AttributeName": "repositorykey", "AttributeType": "S"}],
        ProvisionedThroughput={"ReadCapacityUnits": 10, "WriteCapacityUnits": 10},
    )
    waiter = dynamodb.get_waiter("table_exists")
...
def dynamodb_check_create_tables(db_file):
    try:
        dynamodb.describe_table(TableName=f"{db_file}-packages")
    except:
        logger.info(
            f"DynamoDB: Packages table not found in DynamoDB, creating {db_file}-packages..."
        )
        dynamodb_create_packages_table(db_file)
    try:
        dynamodb.describe_table(TableName=f"{db_file}-repositories")
    except:
        logger.info(
            f"DynamoDB: Repositories table not found in DynamoDB, creating {db_file}-repositories..."
        )
...
def dynamodb_reset_fetched_packages(repository, db_file):
    ...
    dynamodb.update_item(
        ...
        AttributeUpdates={
            "all_versions_fetched": {"Value": {"N": "0"}},
            "all_versions_published": {"Value": {"N": "0"}},
        },
    )
...
def dynamodb_wipe_tables(db_file):
    ...
    try:
        dynamodb.delete_table(TableName=f"{db_file}-packages")
        waiter = dynamodb.get_waiter("table_not_exists")
        waiter.wait(TableName=f"{db_file}-packages")
    except:
        logger.info(f"DynamoDB: {db_file}-packages not found during wipe operation")
    try:
        dynamodb.delete_table(TableName=f"{db_file}-repositories")
        waiter = dynamodb.get_waiter("table_not_exists")
        waiter.wait(TableName=f"{db_file}-repositories")
    except:
...
def dynamodb_insert_package_version(package, repository, version, db_file):
    ...
    dynamodb.put_item(
        ...
        Item={
            ...
            "package": {"S": package},
            "repository": {"S": repository},
            "repositorypackage": {"S": repository + "/" + package},
            "version": {"S": version},
            "all_versions_fetched": {"N": "0"},
            "codeartifact_published": {"N": "0"},
            "all_versions_published": {"N": "0"},
...
def dynamodb_set_all_versions_fetched(package, repository, db_file):
    ...
    dynamodb.update_item(
        TableName=f"{db_file}-packages",
        Key={"packagekey": {"S": packagekey}},
        AttributeUpdates={"all_versions_fetched": {"Value": {"N": "1"}}},
    )
...
def dynamodb_set_all_versions_published(package, repository, db_file):
    ...
    dynamodb.update_item(
        TableName=f"{db_file}-packages",
        Key={"packagekey": {"S": packagekey}},
        AttributeUpdates={"all_versions_published": {"Value": {"N": "1"}}},
    )
...
def dynamodb_check_package_exists(package, repository, db_file):
    ...
    try:
        query = dynamodb.get_item(
            TableName=f"{db_file}-packages", Key={"packagekey": {"S": packagekey}}
        )
        if query["Item"]:
            return True
        else:
...
def dynamodb_set_package_version_to_published(package, repository, version, db_file):
    ...
    dynamodb.update_item(
        TableName=f"{db_file}-packages",
        Key={"packagekey": {"S": packagekey}},
        AttributeUpdates={"codeartifact_published": {"Value": {"N": "1"}}},
    )
...
def dynamodb_set_publish_fail(package, repository, version, db_file):
    ...
    dynamodb.update_item(
        TableName=f"{db_file}-packages",
        Key={"packagekey": {"S": packagekey}},
        AttributeUpdates={"publish_failed": {"Value": {"N": "1"}}},
    )
...
def dynamodb_set_publish_error(package, repository, version, publish_error, db_file):
    ...
    dynamodb.update_item(
        TableName=f"{db_file}-packages",
        Key={"packagekey": {"S": packagekey}},
        AttributeUpdates={"publish_error": {"Value": {"S": publish_error}}},
    )
...
def dynamodb_check_package_version_publish_failed(
...
def dynamodb_fetch_all_versions(package, repository, db_file):
    ...
    response = dynamodb.query(
        ...
        ExpressionAttributeValues={
            ":v_repositorypackage": {"S": repository + "/" + package}
        },
    )
